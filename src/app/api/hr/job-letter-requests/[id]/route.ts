import { reviewJobLetterRequest } from "@/lib/job-letter-request-service";
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    const updated = await reviewJobLetterRequest({
      requestId: id,
      status: body.status,
    ...
    const statusCode = message === "Job letter request not found." ? 404 : 400;
